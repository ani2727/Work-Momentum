...
const Contact = () => {
  ...
  const [popupImage, setPopupImage] = useState(
    'https://res.cloudinary.com/dkdslxqqx/image/upload/v1717658764/iwlasfaj6duzq9qw55hy.webp'
  );
  const [header, setHeader] = useState('');
  ...
  const openPopup = (head, message, imageUrl = popupImage) => {
    setPopupMessage(message);
    setPopupImage(imageUrl);
    setHeader(head);
    setPopupOpen(true);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      openPopup(
        ...
        'https://res.cloudinary.com/dkdslxqqx/image/upload/v1718458077/404-tick_e51zjo.png'
      );
    } catch (err) {
      openPopup(
        'Sorry',
        'Try again',
        'https://png.pngtree.com/png-vector/20221215/ourmid/pngtree-wrong-icon-png-image_6525689.png'
      );
    }
  };
